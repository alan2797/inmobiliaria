...
// import { productoData } from "./data";
import { useStyles } from "./style/style";

const Producto = ({ history }) => {
  ...
  const eliminarProducto = async (data) => {
    console.log("[producto] id", data);
    try {
      const result = await RequestServer.DELETE(
        ROUTE_PARAM(API.PRODUCTO.EDIT, { id: data })
      );
      ...
      <Box mt={2}>
        <EnhancedTable
          ...
          transformColumn={{
            foto: (item) => {
              return (
                <Card className={className.rootCard}>
                  <CardMedia
